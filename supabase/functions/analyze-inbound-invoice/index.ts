...
import { sha256Hex, uploadGeminiFile, waitForGeminiFile } from "../_shared/geminiFiles.ts";
...
Retorne SOMENTE JSON válido: {documentKind,isConsumerInvoice,invoice:{accessKey,number,series,issuedAt,entryExitAt,operationNature,model,protocol,additionalInfo,totalProducts,totalInvoice,freight,discount,insurance,otherExpenses,ipi,icms,icmsSt},issuer:{legalName,tradeName,taxId,stateRegistration,address},recipient:{legalName,taxId},items:[{itemNumber,productCode,productDescription,ean,ncm,cest,cfop,unit,quantity,unitCost,totalCost,discountValue,freightValue,insuranceValue,otherExpensesValue,ipiPercent,ipiValue,icmsPercent,icmsBaseValue,icmsValue,icmsStPercent,icmsStBaseValue,icmsStValue}],warnings,confidence}.`;
    ...
    let fileMode: "files" | "inline" = "inline";
    try {
      const geminiUploadStartedAt = Date.now();
      fileReference = await uploadGeminiFile(bytes, mimeType, fileName, apiKey, fileSha256);
      geminiFileUploadMs = Date.now() - geminiUploadStartedAt;
      const processingStartedAt = Date.now();
      fileReference = await waitForGeminiFile(fileReference, apiKey);
      geminiFileProcessingMs = Date.now() - processingStartedAt;
      fileMode = "files";
    } catch (filesError: any) {
      console.warn("[analyze-inbound-invoice] Gemini Files fallback to inlineData", { message: filesError?.message || "unknown error" });
    }
    const inferenceStartedAt = Date.now();
    const parts = fileReference
      ? [{ text: prompt }, { fileData: { mimeType, fileUri: fileReference.fileUri } }]
      : [{ text: prompt }, { inlineData: { mimeType, data: base64 } }];
    const response = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=" + encodeURIComponent(apiKey), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ parts }], generationConfig: { responseMimeType: "application/json", temperature: 0 } }) });
    ...
    console.log("[analyze-inbound-invoice] completed", { itemCount: extraction.items.length, hasAccessKey: Boolean(extraction.invoice.accessKey), needsReview: extraction.needs_review, fileMode, fileSha256, mimeType, fileSize: bytes.byteLength, extractionSchemaVersion: EXTRACTION_SCHEMA_VERSION, timings: { file_validation_ms: Date.now() - startedAt, storage_upload_ms: storageUploadMs, gemini_file_upload_ms: geminiFileUploadMs, gemini_file_processing_ms: geminiFileProcessingMs, gemini_inference_ms: geminiInferenceMs, structured_output_validation_ms: structuredOutputValidationMs, total_processing_ms: Date.now() - startedAt } });
    return new Response(JSON.stringify({ success: true, extraction, validation: extraction.validation, confidence: extraction.confidence?.accessKey?.confidence || 0, needs_review: extraction.needs_review, sources: { ocr: false, barcode: false, gemini: true, geminiFiles: fileMode === "files" }, documentPath: path, documentMime: mimeType, extractionSchemaVersion: EXTRACTION_SCHEMA_VERSION, sourceFileSha256: fileSha256 }), { headers: { ...cors, "Content-Type": "application/json" } });
  } catch (error: any) {
