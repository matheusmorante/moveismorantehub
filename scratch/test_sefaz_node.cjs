...
function buildSoapEnvelope(cnpj, nsu, tpAmb = '1') {
  const paddedNsu = String(nsu).padStart(15, '0');
...
async function main() {
  ...
  const { data, error } = await supabase.from('settings').select('data').eq('id', 'app').single();
  if (error || !data) {
  ...
  console.log('CNPJ:', companyCnpj);
  console.log('Certificado carregado, tamanho Base64:', certificateBase64.length);
  ...
  console.log('Certificado PEM e Chave PEM extraídos com sucesso.');

  const cleanCnpj = companyCnpj.replace(/\D/g, '');
  const soapEnvelope = buildSoapEnvelope(cleanCnpj, '0', '1');
  ...
  const req = https.request({
    ...
    console.log('HTTP Status SEFAZ:', res.statusCode);
    let body = '';
    res.on('data', chunk => body += chunk);
    res.on('end', () => {
      console.log('SEFAZ Response Length:', body.length);
      console.log('SEFAZ Response Snippet:', body.substring(0, 500));
    });
  });
