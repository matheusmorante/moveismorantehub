...
async function testOptimizedQuery(period) {
    const { current, prev } = getIntervals(period);
    // Buscamos apenas os pedidos desde prev.start até current.end
    const startDateIso = prev.start.toISOString();
    const endDateIso = current.end.toISOString();

    const start = performance.now();
    // Colunas enxutas estritamente necessárias
    const orderSelect = `
        id, order_number, order_index, status, order_type, customer_id, customer_name,
        total_amount, marketing_origin, scheduled_date, delivery_status,
        deleted, created_at,
        order_items(product_id, variation_id, description, quantity, unit_price, unit_discount, cost_price, handling_type, is_temporary_product)
    `;
    ...
    const bytes = new TextEncoder().encode(JSON.stringify(rawRows)).byteLength;
    ...
    const orders = rawRows.map(r => ({
        ...
        date: r.created_at,
        marketingOrigin: r.marketing_origin,
        deleted: r.deleted ?? false,
        shipping: { distance: 0, scheduling: { date: r.scheduled_date } },
    ...
    console.log(`\n=== OTIMIZADO PARA ${period.toUpperCase()} ===`);
    console.log(`Linhas retornadas do banco: ${rawRows.length}`);
    console.log(`Payload: ${(bytes / 1024).toFixed(2)} KB (vs 3401.37 KB antes) -> Redução de ${(100 - (bytes / (3401.37 * 1024) * 100)).toFixed(1)}%`);
    console.log(`Tempo: ${elapsed.toFixed(0)} ms`);
    console.log(`Pedidos período: ${filtered.length} (anterior: ${prevFiltered.length})`);
...
async function main() {
    await testOptimizedQuery('today');
    await testOptimizedQuery('month');
    await testOptimizedQuery('year');
}
