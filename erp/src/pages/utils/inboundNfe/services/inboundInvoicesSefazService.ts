...
import { saveLastInboundInvoiceSyncAt } from './inboundInvoicesCache';
import { checkInboundInvoiceKeyExists } from './inboundInvoicesQueryService';
...
export const consultInboundInvoiceByAccessKey = async (accessKey: string): Promise<{ invoice?: InboundInvoice; rawXml?: string; message?: string }> => {
    ...
    // 3. Se retornou o XML do documento
    if (data.document?.xml) {
        const parsed = parseInboundNfeXml(data.document.xml);
        let finalMessage = data.message;
        
        if (data.document.kind === 'summary') {
            finalMessage = 'A SEFAZ liberou apenas o Resumo da Nota (sem os itens). Para obter os itens, você precisa realizar a Manifestação (Ciência da Operação) ou enviar o arquivo XML diretamente.';
        }
        
        return { invoice: parsed, rawXml: data.document.xml, message: finalMessage };
    }
