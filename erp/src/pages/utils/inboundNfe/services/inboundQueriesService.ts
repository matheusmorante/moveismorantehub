...
import {
    getCurrentYearStr, getPreviousYearStr, getYearDateBounds,
    getCurrentYearMonthStr, getPreviousYearMonthStr, getMonthDateBounds
} from './inboundDateUtils';
import { toInboundInvoiceStatus, normalizeAdditionalCost, normalizeAdditionalCosts, normalizeInvoiceItem, isSampleInvoice } from './inboundNormalizationService';
        ...
        if (!error && data) {
            ...
            try {
                ...
                if (!itemError && itemRows && itemRows.length > 0) {
                    ...
                    result.items = result.items.map((r: any, idx: number) => normalizeInvoiceItem(r, idx));
                }
            } catch (err) {
