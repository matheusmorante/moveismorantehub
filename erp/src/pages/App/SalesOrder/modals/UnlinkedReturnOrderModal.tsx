...
const UnlinkedReturnOrderModal = ({ onClose, onSaveSuccess }: Props) => {
    ...
    const saveReturn = async () => {
        ...
        const populatedItems = validItems.map(item => ({
            ...
            returnedUnitPrice: item.unitPrice,
            returnedTotalValue: item.quantity * item.unitPrice,
            originalUnitPrice: item.unitPrice,
            originalTotalValue: item.quantity * item.unitPrice,
        }));
        ...
        const order: Order = {
            ...
            returnedTotalAmount: summary.itemsTotalValue,
            originalSoldTotal: summary.itemsTotalValue,
            returnKind: 'complete',
            observation: observations.join('\n'),
            collectionObservation: collectAtAddress ? observations.join('\n') : undefined,
            itemsSummary: summary,
            payments: [],
            paymentsSummary: { totalPaymentsFee: 0, totalOrderValue: summary.itemsTotalValue, totalPaid: 0, totalAmountPaid: 0, amountRemaining: 0 },
            shipping: { value: 0, deliveryMethod: collectAtAddress ? 'delivery' : 'pickup', orderType: 'Standard', scheduling: collectAtAddress ? scheduling : { ...EMPTY_SCHEDULING, notInformed: true } },
    ...
    return <div className="fixed inset-0 z-[999999] flex items-stretch justify-center bg-slate-900/60 p-0 backdrop-blur-sm xl:items-center xl:p-4" onClick={onClose}>
        <div className="flex h-full w-full max-w-none flex-col overflow-hidden border border-slate-100 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 xl:h-auto xl:max-h-[90vh] xl:max-w-2xl xl:rounded-3xl" onClick={event => event.stopPropagation()}>
            ...
            <div className="custom-scrollbar flex-1 space-y-6 overflow-y-auto p-6">
                <section><div className="mb-3 flex items-center justify-between"><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Cliente *</label><button type="button" onClick={() => setNewCustomerOpen(true)} className="text-[10px] font-black uppercase tracking-widest text-blue-600"><i className="bi bi-person-plus mr-1" />Novo cliente</button></div><div className="relative"><i className="bi bi-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" /><input value={customerSearchTerm} onFocus={() => setCustomerSearchOpen(canSearchCustomers(customerSearchTerm))} onChange={event => { const value = event.target.value; setCustomerSearchTerm(value); setCustomer(EMPTY_CUSTOMER); setCustomerSearchOpen(canSearchCustomers(value)); }} placeholder="Digite ao menos 2 caracteres para buscar" className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-11 pr-4 text-sm font-bold outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-950" />{customer.fullName && <span className="mt-2 block text-[10px] font-black uppercase tracking-widest text-emerald-600"><i className="bi bi-check-circle-fill mr-1" />Cliente selecionado</span>}</div></section>
