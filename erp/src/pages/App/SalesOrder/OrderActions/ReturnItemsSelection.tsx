...
const ReturnItemsSelection = ({ order, quantities, onToggle, onQuantityChange }: Props) => <div>
    ...
    <div className="space-y-3">{order.items.map((item, index) => {
        ...
        return <div key={`${itemId}-${index}`} className={`flex flex-col rounded-3xl border p-4 transition-all ${selected ? "border-amber-200 bg-amber-50/50 shadow-premium-sm dark:border-amber-900/40 dark:bg-amber-900/10" : "border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-800"}`}>
            <div className="flex items-center justify-between gap-4"><button type="button" onClick={() => onToggle(itemId, item.quantity)} className="flex flex-1 items-center gap-3 text-left outline-none"><div className={`flex h-6 w-6 items-center justify-center rounded-lg border-2 ${selected ? "border-amber-500 bg-amber-500 text-white" : "border-slate-200 dark:border-slate-700"}`}>{selected && <i className="bi bi-check-lg text-xs" />}</div><div><span className={`text-[11px] font-black uppercase leading-tight ${selected ? "text-amber-700 dark:text-amber-400" : "text-slate-700 dark:text-slate-200"}`}>{item.description}</span><span className="mt-1 block text-[9px] font-bold uppercase tracking-widest text-slate-400">Total original: {item.quantity} un</span></div></button><strong className={`text-xs ${selected ? "text-amber-600" : "text-slate-900 dark:text-white"}`}>R$ {((selected ? quantities[itemId] : item.quantity) * item.unitPrice).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</strong></div>
