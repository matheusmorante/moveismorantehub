...
const ReturnConfirmationModal = ({ onCancel, onConfirm }: ReturnConfirmationModalProps) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
            <button
                type="button"
                aria-label="Fechar confirmação de devolução"
                className="fixed inset-0 bg-slate-950/55 transition-opacity"
                onClick={onCancel}
            />
            <section role="dialog" aria-modal="true" aria-labelledby="return-confirmation-title" className="relative z-10 w-full max-w-lg rounded-2xl border border-amber-200 bg-white p-6 shadow-2xl dark:border-amber-900/70 dark:bg-slate-900" onClick={(event) => event.stopPropagation()}>
                ...
                <p className="mt-3 text-sm leading-relaxed text-slate-600 dark:text-slate-300">A movimentação de saída original permanece preservada. Quando a devolução for atendida, a entrada correspondente será registrada no estoque <strong>apenas para produtos cadastrados</strong>. Itens sem cadastro no sistema não geram movimentação de estoque.</p>
