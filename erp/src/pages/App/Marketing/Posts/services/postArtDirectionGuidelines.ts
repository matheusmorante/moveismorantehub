/**
 * postArtDirectionGuidelines.ts — Diretrizes Globais de Direção de Arte e Qualidade Publicitária.
 *
 * REGRA CENTRAL:
 * Separar definitivamente dois conceitos:
 * 1. FIDELIDADE DO PRODUTO = extremamente rígida (fotos reais fornecidas são a fonte visual de verdade).
 * 2. DIREÇÃO DE ARTE AO REDOR DO PRODUTO = criativa, rica, sofisticada e profissional.
 *
 * O fato de o produto precisar ser fiel às imagens originais NÃO significa
 * que o restante da arte deva ser simples, chapado ou parecer uma montagem automática.
 */

export const GLOBAL_ART_DIRECTION_SECTION = `==================================================
DIREÇÃO DE ARTE OBRIGATÓRIA (QUALIDADE PUBLICITÁRIA PREMIUM)
==================================================

Crie uma peça publicitária visualmente rica, profissional e sofisticada, com qualidade de campanha de varejo produzida por designer gráfico experiente e diretor de arte.

O resultado NÃO deve parecer uma montagem simples de catálogo, template genérico, card básico de e-commerce ou produto apenas recortado e colado sobre um fundo chapado.

Construa uma composição publicitária completa e envolvente ao redor do produto.

DIRETRIZES DE COMPOSIÇÃO:
- Ambientação comercial realista e elegante relacionada ao ambiente de uso do móvel:
  * Guarda-roupa / Cômoda: quarto contemporâneo sofisticado;
  * Sofá: sala de estar elegante e acolhedora;
  * Mesa / Cadeiras: sala de jantar moderna;
  * Balcão / Paneleiro / Modulados: cozinha contemporânea limpa e iluminada.
  O cenário não deve competir com o produto: use decoração secundária (tapetes, piso, parede, plantas, iluminação) para contextualizar, criar profundidade e valorizar a peça.
- Iluminação publicitária de estúdio: luz de recorte (rim light), sombras de contato e projeção realistas no piso/parede, profundidade e múltiplos planos visuais.
- Elementos gráficos e de design: utilize, com harmonia e controle, formas gráficas, molduras, faixas, degradês elegantes, texturas discretas, recortes e detalhes gráficos premium.
- Hierarquia tipográfica forte: integração visual equilibrada entre fotografia e tipografia.
- Espaço negativo intencional: sofisticado não significa encher de elementos. Use riqueza visual com controle, elegância e hierarquia clara.

HIERARQUIA VISUAL COMERCIAL (ORDEM DE ATENÇÃO):
1. PRODUTO (Protagonista absoluto da peça);
2. OFERTA / PREÇO (Bloco de preço publicitário destacado, com card/faixa contrastante, detalhes da marca e separação elegante de valor e moeda);
3. NOME DO PRODUTO (Com alta legibilidade e peso tipográfico equilibrado);
4. DIFERENCIAIS COMERCIAIS AUTORIZADOS (Mantenha APENAS os benefícios comerciais reais da loja: "Entrega rápida em até 5 dias úteis", "Montagem inclusa com equipe especializada" e "Frete grátis até 100km". Não invente outros);
5. CONDIÇÃO DE PAGAMENTO (Parcelamento claro e atraente);
6. INFORMAÇÕES SECUNDÁRIAS (Foto do móvel aberto/interno em card discreto, miniaturas reais de outras cores — NUNCA duplicar a cor principal na galeria de cores);
7. MARCA (Logo oficial da Móveis Morante posicionado com nobreza e fidelidade).

IDENTIDADE VISUAL DA MÓVEIS MORANTE:
- Cores de identidade: azul escuro institucional, amarelo/dourado vibrante de destaque, branco e tons neutros contemporâneos (cinzas elegantes, amadeirados suaves). Paleta oficial da peça: fundo azul marinho institucional (#002B49), tipografia e elementos em branco puro e amarelo ouro vibrante (#F7B731 / #FFC107).
- Aplique essas cores estrategicamente no bloco de preço, faixas, molduras, ícones, traçados e destaques tipográficos.
- O logo da Móveis Morante e os selos oficiais são ASSETS PRONTOS: nunca redesenhe ou recrie por aproximação.

SLOGANS AUTORIZADOS DO PRODUTO (REGRAS ESTRITAS):
- São permitidos EXCLUSIVAMENTE 2 slogans/destaques específicos do produto (móvel):
  1. SLOGAN 1 (ABAIXO DO TÍTULO DO PRODUTO): frase curta de destaque do móvel, posicionada logo abaixo do nome do produto.
  2. SLOGAN 2 (AO LADO DO PRODUTO): frase elegante posicionada ao lado do móvel, com tipografia em estilo caligráfico/escrita à mão (handwritten/brush) e uma linha traçada amarela destacada logo abaixo (sublinhado caligráfico amarelo).

TEXTOS, SLOGANS E RÓTULOS GENÉRICOS EXPRESSAMENTE PROIBIDOS:
- É expressamente PROIBIDO inserir slogan da empresa ou da loja (ex.: "Seu lar mais completo com a Móveis Morante" ou "Qualidade que cabe no seu bolso") no canto inferior direito ou em qualquer parte da arte. A logo da Móveis Morante já carrega a identidade oficial da empresa.
- É expressamente PROIBIDO à IA adicionar qualquer elemento, texto, caixa ou selo adicional que não tenha sido solicitado nesta especificação.
- É expressamente PROIBIDO inventar slogans publicitários ou frases clichês como "mais organização para o seu dia", "seu lar merece", etc.
- É expressamente PROIBIDO inserir rótulos, tags ou balões sobrepostos ou ao redor da foto de visão interna (espaço interno), tais como:
  * "material de qualidade";
  * "amplo espaço interno";
  * "design moderno";
  * "qualidade superior";
  * "espaço inteligente".
- A foto da visão interna (móvel aberto) deve ser exibida de forma limpa, elegante e puramente fotográfica: sem textos sobrepostos, sem setas e sem tags descritivas.

ANTI-SIMPLICIDADE (RESULTADO INSUFICIENTE PROIBIDO):
Não produza uma composição semelhante a:
"produto recortado + fundo branco/liso + caixa branca simples + preço".
Isso é considerado resultado insuficiente.

Evite terminantemente:
- Layout excessivamente plano e chapado;
- Grandes áreas vazias sem função ou intenção estética;
- Caixas brancas genéricas de texto estilo PowerPoint;
- Aparência de flyer montado automaticamente por aplicativo básico;
- Produto isolado flutuando no ar sem sombras nem chão/parede;
- Tipografia fraca ou sem hierarquia de peso e tamanho.

REFERÊNCIA DE QUALIDADE:
Campanha publicitária premium de varejo de móveis para redes sociais, com direção de arte contemporânea, fotografia de produto valorizada, ambientação elegante, composição editorial e forte hierarquia comercial.`;

export const VISUAL_GROUNDING_MANDATORY_RULE = `==================================================
REGRA CRÍTICA — ANCORAGEM OBRIGATÓRIA DAS REFERÊNCIAS VISUAIS
==================================================

1. EXTRAÇÃO E IDENTIFICAÇÃO PRELIMINAR:
Ao receber este pacote (seja colado no chat ou links oficiais), extraia e identifique explicitamente quais arquivos são as IMAGENS DO PRODUTO (product/primary.*, product/open-view.*, product/variations/*) e ASSETS OFICIAIS (official-assets/logo.*) antes de qualquer tentativa de geração.
O usuário cola ou anexa as fotos oficiais e assets no chat (Ctrl+V) junto com este prompt.
• IMAGEM PRINCIPAL: Fotografia do móvel fechado na cor principal (protagonista da arte).
• VISÃO INTERNA / ABERTO (se enviada): Fotografia do móvel aberto para entender divisórias reais.
• LOGO MÓVEIS MORANTE (se enviado): Asset oficial pronto. NUNCA redesenhar ou recriar.
• DEMAIS IMAGENS (OUTRAS CORES): Miniaturas reais para galeria de cores.
  REGRA OBRIGATÓRIA: NUNCA inclua a Variação 1 (cor principal) na galeria de cores secundárias.

2. ANCORAGEM COMO REFERÊNCIA VISUAL DIRETA:
As imagens identificadas DEVEM ser efetivamente anexadas e utilizadas como REFERÊNCIAS VISUAIS DIRETAS na chamada de geração da imagem, e não apenas analisadas ou descritas anteriormente.
É expressamente PROIBIDO gerar a arte apenas a partir de uma descrição textual do produto quando existem fotografias reais no pacote.

3. CONFIRMAÇÃO INTERNA OBRIGATÓRIA ANTES DE GERAR:
Antes de chamar o gerador de imagem, confirme internamente:
- Qual é o produto exato do pacote?
- Quais arquivos locais ou URLs são as suas fotografias oficiais?
- Essas fotografias estão efetivamente vinculadas como input de referência visual para o gerador de imagem?

4. BLOQUEIO DE GERAÇÃO SE NÃO HOUVER VÍNCULO VISUAL:
Se as fotografias não puderem ser fornecidas ao gerador de imagem como referência visual direta, NÃO GERE A IMAGEM. Informe ao usuário que não foi possível utilizar as referências do pacote.
Nunca substitua uma referência ausente por conhecimento próprio, produto semelhante ou objeto aleatório.
Se o pacote contém um guarda-roupa, uma geração contendo relógio, carteira, sofá ou qualquer outro produto diferente é uma falha crítica, não uma interpretação criativa.

5. CONTINUIDADE DA SESSÃO E REUTILIZAÇÃO DAS REFERÊNCIAS:
Após analisar o pacote, mantenha as imagens extraídas do produto vinculadas à tarefa até a geração final. Se o usuário responder apenas informando o formato (ex: "4:5" ou "Feed"), isso NÃO inicia uma nova interpretação visual. Reutilize obrigatoriamente as mesmas imagens do produto identificadas na etapa anterior.`;

export const DIRECT_WORKFLOW_FORMAT_INSTRUCTION = `==================================================
FORMATO ALVO E FLUXO DIRETO (SEM INTERRUPÇÕES)
==================================================
FORMATO PADRÃO SOLICITADO: FEED (Aspect Ratio 4:5 — 1080 × 1350)
(Se o usuário solicitar explicitamente Story/Status 9:16, utilize 1080 × 1920).

FLUXO CONTÍNUO EM UMA ÚNICA ETAPA:
O formato padrão já está definido como Feed 4:5. NÃO pare a execução para perguntar qual formato deseja.
Execute o fluxo contínuo em sequência única:
Leitura do pacote → Ancoragem das fotos reais como referências visuais → Geração direta da arte publicitária.`;

