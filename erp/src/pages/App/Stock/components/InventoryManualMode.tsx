                        ...
                        <div key={item.id} className={`bg-white dark:bg-slate-800 border ${isCounted ? 'border-emerald-500/30 dark:border-emerald-500/20' : 'border-slate-200 dark:border-slate-700'} rounded-xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 transition-colors`}>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    <h4 className="font-bold text-slate-800 dark:text-slate-100 truncate text-base">{item.name}</h4>
                                    {isCounted && <div className="w-2 h-2 rounded-full bg-emerald-500"></div>}
                                </div>
                                <div className="text-xs text-slate-500 dark:text-slate-400 truncate mb-3">{item.supplierNames}</div>
                            ...
                            <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900/50 p-2 rounded-xl sm:shrink-0 w-full sm:w-auto justify-between sm:justify-start">
                                <button
                                    onClick={() => onUpdateCount(item.id, Math.max(0, (item.physicalCount || 0) - 1))}
                                    disabled={item.physicalCount === 0}
                                    className="w-12 h-12 flex items-center justify-center rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed text-xl shadow-sm active:scale-95 transition-all"
                                >
                                ...
                                <div className="flex flex-col items-center min-w-[80px]">
                                    <input
                                        ...
                                        placeholder="-"
                                        className="w-full text-center bg-transparent text-2xl font-black text-slate-800 dark:text-slate-100 outline-none p-0 focus:ring-0 border-0"
                                    />
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Contado</span>
                                </div>

                                <button
                                    onClick={() => onUpdateCount(item.id, (item.physicalCount || 0) + 1)}
                                    className="w-12 h-12 flex items-center justify-center rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 text-xl shadow-sm active:scale-95 transition-all"
                                >
