...
interface StockListProps {
    readonly onLaunch: (product: Product, variation?: Variation) => void;
}

export const StockList: React.FC<StockListProps> = ({ onLaunch }) => {
            ...
            <div className="p-6 border-b border-slate-50 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-0 z-30 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="relative flex-1 max-w-md">
                    ...
                    <button
                        ...
                        className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-all cursor-pointer"
            ...
            <div className="overflow-x-auto overflow-y-auto max-h-[60vh] custom-scrollbar">
                <table className="w-full text-left border-collapse whitespace-nowrap">
                    <thead className="sticky top-0 z-20 bg-slate-50 dark:bg-slate-950">
                        <tr>
                            <th scope="col" className="px-4 md:px-8 py-3 md:py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800">Item / Produto</th>
                            <th scope="col" className="px-4 md:px-8 py-3 md:py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800 text-center">Saldo Atual</th>
                            <th scope="col" className="px-4 md:px-8 py-3 md:py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800 text-right">Ações</th>
                        </tr>
                    </thead>
                                            ...
                                            <div className="relative shrink-0">
                                                ...
                                                {product.hasVariations && (
                                                    <div className="absolute -top-1 -right-1 w-4 h-4 bg-indigo-600 text-white rounded-md flex items-center justify-center shadow-lg border-2 border-white dark:border-slate-800 animate-in zoom-in duration-300">
                                                        <i className="bi bi-layers-fill text-[8px]" aria-hidden="true" />
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        {!product.hasVariations && (
                                            <>
                                                <Link
                                                    ...
                                                    aria-label="Imprimir Etiquetas deste Produto"
                                                >
                                    ...
                                    <tr key={v.id || `var-${vIdx}`} className="bg-slate-50/30 dark:bg-slate-900/30 hover:bg-emerald-50/40 dark:hover:bg-emerald-900/10 transition-colors group border-l-4 border-emerald-500/20">
                                        <td className="px-4 md:px-8 py-3 md:py-4 pl-8 md:pl-16">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center text-slate-300 border border-slate-100 dark:border-slate-800 shrink-0">
                                                    <i className="bi bi-stack text-sm" aria-hidden="true" />
                                                </div>
                                                <div className="flex flex-col min-w-[120px]">
                                        ...
                                        <td className="px-4 md:px-8 py-3 md:py-4 text-right flex items-center justify-end gap-2">
                                            <Link
                                                ...
                                                aria-label="Imprimir Etiquetas desta Variação"
                                            >
                                                <i className="bi bi-printer" aria-hidden="true" />
                                            </Link>
                                            <button
                                                ...
                                                className="inline-flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-emerald-600 rounded-lg transition-all font-black uppercase text-[9px] tracking-widest cursor-pointer"
                                            >
                                                <i className="bi bi-lightning-charge" aria-hidden="true" />
                                                <span className="hidden sm:inline">Lançar</span>
                                            </button>
            ...
            {filtered.length === 0 && (
                <div className="p-20 flex flex-col items-center justify-center text-center">
                    <div className="w-20 h-20 bg-slate-50 dark:bg-slate-900 rounded-[2rem] flex items-center justify-center text-slate-200 dark:text-slate-800 mb-6">
                        <i className="bi bi-search text-4xl" aria-hidden="true" />
                    </div>
