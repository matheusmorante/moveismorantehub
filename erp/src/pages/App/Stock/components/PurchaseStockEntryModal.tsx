...
export default function PurchaseStockEntryModal({ purchase, isOpen, onClose }: Props) {
    ...
    useEffect(() => {
        ...
        setObservation(`Entrada a partir do Pedido #${purchase.id?.slice(-4)} | Fornecedor: ${purchase.supplierName} | Data do pedido: ${purchase.date?.slice(0, 10)}${purchase.observation ? ` | ${purchase.observation}` : ''}`);
        ...
        try {
            for (const item of purchase.items) {
                ...
                await saveInventoryMove({
                    ...
                    label: `Entrada a partir do Pedido #${purchase.id?.slice(-4)}`,
