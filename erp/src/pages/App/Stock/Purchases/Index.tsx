...
/**
 * Página principal do módulo de compras do estoque com filtragem por fornecedor e gerenciamento de status.
 */
export const PurchasesPage: React.FC = () => {
    ...
    useEffect(() => {
        fetchPersons('suppliers').then((data) => setSuppliers(data || []));
        return subscribeToPurchases((data) => {
            setPurchases(data || []);
            setLoading(false);
            setSelectedPurchase((current) =>
                current ? data.find((purchase) => purchase.id === current.id) || current : null
            );
        });
    }, []);
    ...
    const filteredPurchases = useMemo(
        () => (selectedSupplierId ? purchases.filter((purchase) => purchase.supplierId === selectedSupplierId) : []),
    ...
    return (
        <div className="flex flex-col">
            <div className="flex-1 flex flex-col min-w-0">
                <header className="mb-3 flex flex-row items-center justify-between gap-3 sm:mb-4">
                    <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-600 text-white shadow-md sm:h-10 sm:w-10">
                            <i className="bi bi-truck" aria-hidden="true" />
                        </div>
                        <h1 className="text-lg font-black tracking-tight text-slate-800 dark:text-slate-100 sm:text-xl md:text-2xl">
                            Pedidos de Compra
                        </h1>
                    </div>
                    <button
                        ...
                        className="rounded-xl bg-blue-600 px-3.5 py-2 text-xs font-black uppercase tracking-wider text-white shadow-md transition-all hover:bg-blue-700 sm:px-4 sm:py-2.5"
                    >
                ...
                <section className="overflow-hidden rounded-[2.5rem] border border-slate-100 bg-white shadow-2xl shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
                    <div className="flex flex-col gap-4 border-b border-slate-50 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 md:flex-row md:items-center">
