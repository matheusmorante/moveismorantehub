...
import {
    ReceiptBadgeContentResult,
    getReceiptItemsMovementList,
} from './receiptBadgeContent';

interface ReceiptMovementBadgePopoverProps {
    readonly coords: { top: number; left: number; placement: 'top' | 'bottom' };
    readonly content: ReceiptBadgeContentResult;
...
export const ReceiptMovementBadgePopover: React.FC<ReceiptMovementBadgePopoverProps> = ({
    coords,
    content,
    receipt,
    hasMovement,
    ...
    const itemsMovement = getReceiptItemsMovementList(receipt);
    ...
    return createPortal(
        ...
        >
            ...
            <div className="flex items-start justify-between gap-2.5 shrink-0">
                <div className="flex items-center gap-2.5">
                    <div className={`relative flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${content.badgeColorClass}`}>
                        ...
                        {isReversed ? (
                            <span className="absolute -top-1 -right-1 flex h-3 w-3 items-center justify-center rounded-full bg-red-800 text-white shadow-2xs ring-1 ring-white dark:ring-slate-900 pointer-events-none">
                                <RotateCcw className="h-2 w-2 stroke-[3]" />
                            </span>
                        ) : hasMovement ? (
            ...
            {/* Lista com Itens do Recebimento */}
            {itemsMovement.length > 0 && (
                <div className="mt-3 pt-2.5 border-t border-slate-100 dark:border-slate-800 flex flex-col min-h-0">
                    <div className="grid grid-cols-[minmax(0,1fr)_92px] gap-2 mb-2 shrink-0">
                        <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">
                            Itens do Recebimento ({itemsMovement.length})
                        </span>
                    ...
                    <div className="overflow-y-auto max-h-48 space-y-1.5 pr-1 custom-scrollbar">
                        {itemsMovement.map((item, idx) => (
                            <div
                                key={idx}
                                className="grid grid-cols-[minmax(0,1fr)_92px] items-center gap-2 p-2 rounded-xl bg-slate-50/80 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700/60 text-[11px]"
                            >
