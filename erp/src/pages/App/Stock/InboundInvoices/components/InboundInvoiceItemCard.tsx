...
export interface RealProductSuggestion {
...
interface InboundInvoiceItemCardProps {
    ...
    readonly realSuggestion?: RealProductSuggestion;
    readonly isRejected: boolean;
    readonly isProcessingSuggestions: boolean;
    readonly onSelectProduct: (itemNumber: number, product: Product, variation?: Variation) => void;
    readonly onUnlinkProduct: (itemNumber: number) => void;
    readonly onAcceptRealSuggestion: (itemNumber: number, suggestion: RealProductSuggestion) => void;
    readonly onRejectSuggestion: (itemNumber: number) => void;
    readonly onRequestIndividualCreation: (item: InboundInvoiceItem) => void;
}
                        ...
                        <div className="space-y-1">
                            ...
                            {linked && (
                                <p className="text-[10px] font-mono text-slate-400">
                                    Código / SKU: {item.linkedProductCode || '—'}
                                </p>
                            )}
                        </div>
                        ...
                        {!linked && (
                            <div className="flex justify-end pt-1">
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700 hover:bg-emerald-100 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300 transition-colors shadow-sm cursor-pointer"
                                >
                                    <i className="bi bi-plus-circle-fill text-xs" aria-hidden="true" />
...
export default InboundInvoiceItemCard;
