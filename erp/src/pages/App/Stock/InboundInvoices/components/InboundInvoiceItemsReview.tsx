...
import { useInboundInvoiceItemsReview, INBOUND_SUGGESTIONS_FEATURE_ENABLED } from '../hooks/useInboundInvoiceItemsReview';

export interface InboundInvoiceItemsReviewProps {
...
export function InboundInvoiceItemsReview(props: InboundInvoiceItemsReviewProps) {
    const { items, supplierId, suggestionsEnabled = true } = props;
        ...
        <>
            <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
                    ...
                    {INBOUND_SUGGESTIONS_FEATURE_ENABLED && suggestionsEnabled && (
                        <button
                            type="button"
            ...
            {/* Modal: Cadastro Individual com Markup */}
            ...
            {/* Modal de Cadastro Rápido de Variações */}
