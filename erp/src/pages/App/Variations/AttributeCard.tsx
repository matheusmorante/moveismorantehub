                ...
                <button type="button" onClick={() => setIsExpanded((current) => !current)} className="min-w-0 flex-1 flex items-center justify-between gap-4 text-left cursor-pointer" aria-expanded={isExpanded}>
                    <span className="min-w-0">
                        ...
                        <span className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mt-0.5">
                            {attribute.options.length} {attribute.options.length === 1 ? 'valor' : 'valores'}
                        </span>
                    </span>
                ...
                <div className="relative shrink-0">
                    ...
                    {menuOpen && (
                        <div className="absolute right-0 top-11 z-30 w-52 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl p-1.5">
                            ...
                            <button type="button" onClick={() => { setMenuOpen(false); setIsExpanded(true); setIsAdding(true); }} className="w-full px-3 py-2 text-left rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer">
                                <i className="bi bi-plus-circle mr-2" aria-hidden="true" />Adicionar valores
                            </button>
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 px-5 py-5 sm:px-6 space-y-4">
                    ...
                    {!isAdding ? (
                        <button type="button" onClick={() => setIsAdding(true)} className="text-xs font-black text-blue-600 hover:text-blue-700 cursor-pointer">
                            <i className="bi bi-plus-lg mr-1" aria-hidden="true" />Adicionar valor
                        </button>
                    ) : (
