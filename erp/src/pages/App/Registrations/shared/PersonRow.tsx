...
const getRoleBadge = (role?: string) => {
    switch (role) {
        ...
        case 'accountant':
            return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider bg-cyan-50 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800"><i className="bi bi-calculator-fill text-[10px]" />Contador</span>;
        case 'pending':
                ...
                return (
                    <td key="id" className="px-3 py-1.5 text-left">
                        <span className="font-mono text-xs text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg">
                            {person.type === 'employees' ? person.employeeCode ?? '-' : person.id || '-'}
                        </span>
                    </td>
                );
            case 'fullName':
