...
export const engineeringImpactMaps: Record<string, EngineeringImpact[]> = {
    ...
    "Pedidos de venda e devoluções": [
        { action: "Venda: Rascunho → Agendado", effects: ["Estoque: saída dos itens reais conforme automação", "Agenda: pedido entra pelo agendamento", "Notificações: venda e montagens são comunicadas"] },
        { action: "Venda: Agendado → Atendido", effects: ["Estoque: completa baixa pendente sem duplicar", "Agenda: pedido deixa de aparecer", "Pós-venda: devolução e avaliação ficam disponíveis"] },
        { action: "Venda → Cancelado", effects: ["Estoque: estorna saídas vinculadas e recompõe saldo", "Agenda: remove pedido", "Mobile: cria notificação de cancelamento"] },
        ...
        { action: "Devolução: Agendado → Atendido", effects: ["Estoque: cria entradas dos itens devolvidos", "Agenda: remove devolução", "Integridade: bloqueia cancelar e desfazer"] },
    ],
    "Orçamentos": [{ action: "Gerar venda", effects: ["Pedidos: cria nova venda independente", "Estoque e agenda: só passam a agir no pedido gerado"] }],
    "Assistências": [{ action: "Agendar assistência", effects: ["Agenda: exibe serviço no período", "Estoque: não usa a baixa automática de venda"] }],
    "Produtos e estoque": [
    ...
    "Agenda": [{ action: "Alterar status ou agendamento", effects: ["Pedidos agendados: entram no período filtrado", "Atendidos e cancelados: saem", "Mobile e ERP: atualização em tempo real"] }],
