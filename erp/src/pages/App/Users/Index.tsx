...
type Profile = AccessProfile & { role: UserRole; };
...
const UsersManagement = () => {
    ...
    useEffect(() => {
        ...
        void supabase.from('profiles').select('id,email,full_name,role').order('email').then(({ data, error }) => {
            if (error) toast.error('Erro ao carregar as contas.'); else setProfiles((data || []) as any as Profile[]);
            setLoading(false);
        });
    }, [isAdmin]);
