...
const Categories = () => {
    ...
    const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            ...
            try {
                ...
                existingCats?.forEach(c => catsMap.set(c.name.trim().toUpperCase(), c.id));
                    ...
                    if (parts.length > 0) {
                        ...
                        if (parentName) {
                            if (catsMap.has(parentName)) {
                                parentId = catsMap.get(parentName)!;
                            } else {
                                parentId = await insertCategorySafe(parentName);
                                catsMap.set(parentName, parentId);
                                createdCatsCount++;
                            }
                        }
                        ...
                        if (childName) {
                            if (catsMap.has(childName)) {
                                childId = catsMap.get(childName)!;
                            } else {
                                childId = await insertCategorySafe(childName);
                                catsMap.set(childName, childId);
                                createdCatsCount++;
                            }
                        }
    ...
    const openAddCategoryToEnv = (envId: string) => {
        setEditingNode(null);
        setNameInput("");
        setSelectedEnvironments([envId]);
        setShowModal("categoria");
    };
    ...
    return (
        <div className="p-4 md:p-8 flex flex-col gap-6 max-w-5xl mx-auto w-full animate-in fade-in duration-300">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                        Estrutura unificada de organização de produtos
                    </p>
                </div>
                <div className="flex items-center gap-3 flex-wrap">
                    <button
                        onClick={() => { setShowModal("ambiente"); }}
                        className="bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center gap-1.5 shadow-sm"
                    >
            ...
            {/* Tabela Unificada de Ambientes e Categorias */}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-100 dark:border-slate-800">
                                <th className="py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Nome do Ambiente</th>
                                <th className="py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Categorias</th>
                            ...
                            {environments.map(env => {
                                ...
                                return (
                                    <tr key={env.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors">
                                        <td className="py-3.5 font-bold text-xs text-slate-850 dark:text-slate-200 uppercase">{env.name}</td>
                                        <td className="py-3.5">
                                            <div className="flex flex-wrap items-center gap-1.5">
                                                {childrenList.map(c => (
                                                    <span key={c.id} className="px-2.5 py-1 bg-slate-100 dark:bg-slate-850 border border-slate-200/60 dark:border-slate-800 rounded-lg text-[10px] font-black text-slate-600 dark:text-slate-300 uppercase flex items-center gap-1">
                                                        {c.name}
                                                        <button 
                                                        ...
                                                        >
                                                            <i className="bi bi-pencil text-[9px]"></i>
                                                        </button>
                                                    </span>
                                                ))}
                                                <button
                                                    onClick={() => openAddCategoryToEnv(env.id)}
                                                    className="w-6 h-6 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/40 flex items-center justify-center transition-colors shadow-2xs"
                                        ...
                                        <td className="py-3.5 text-right">
                                            <div className="flex items-center justify-end gap-1.5">
                                                <button onClick={() => openEdit(env, true)} className="p-1.5 text-blue-500 hover:bg-blue-50 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Editar ambiente">
                                                    <i className="bi bi-pencil"></i>
                                                </button>
                                                <button onClick={() => handleDelete(env.id, true)} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Excluir ambiente">
