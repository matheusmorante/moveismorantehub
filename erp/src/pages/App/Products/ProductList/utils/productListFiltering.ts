...
/** 
 * Aplica os filtros locais e a ordenação pura da lista de produtos sem mutação.
 */
export function filterAndSortProducts(products: readonly Product[], filters: ProductListFilters = {}): Product[] {
    return products
        .filter((product) => {
            ...
            if (!product.parentId) {
                matchesRelatedProduct = (product.variations as unknown as VariationSearchable[] | undefined)
                    ?.some((variation) => matchesSearch(variation.name || variation.displayName)) || false;
