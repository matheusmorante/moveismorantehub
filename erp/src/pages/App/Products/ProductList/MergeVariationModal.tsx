...
export const MergeVariationModal: React.FC<Props> = ({ variation, onClose, onMerged }) => {
    ...
    const confirm = async () => {
        ...
        try {
            ...
            toast.error(error?.message || 'Não foi possível mesclar as variações.');
    ...
    return <div className="fixed inset-0 z-[10050] flex items-center justify-center bg-slate-950/55 p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl dark:bg-slate-900">
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 p-5 dark:border-slate-800">
                <div><h2 className="text-lg font-black text-slate-800 dark:text-white">Mesclar com outra variação</h2><p className="mt-1 text-sm text-slate-500">“{sourceLabel}” continuará com seu UUID histórico e passará a apontar para a variação canônica.</p></div>
            ...
            <div className="flex justify-end gap-3 border-t border-slate-100 p-5 dark:border-slate-800"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-600">Cancelar</button><button type="button" disabled={!target || saving} onClick={confirm} className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-40">{saving ? 'Mesclando…' : 'Confirmar mesclagem'}</button></div>
        </div>
    </div>;
};
