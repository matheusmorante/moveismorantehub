...
export interface CartesianAttributeOption {
    name: string;
    values: string[];
    showName: boolean;
}

export interface CartesianVariationModalProps {
    readonly isOpen: boolean;
    readonly onClose: () => void;
    readonly onGenerate: (attributes: CartesianAttributeOption[]) => void;
    readonly isGenerating: boolean;
}

export const CartesianVariationModal: React.FC<CartesianVariationModalProps> = ({
    isOpen,
    onClose,
    onGenerate,
    isGenerating
    ...
    const [selectedOptions, setSelectedOptions] = useState<CartesianAttributeOption[]>([]);
    ...
    return createPortal(
        <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4">
            <button 
                ...
                className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm cursor-default" 
                onClick={onClose} 
            />
            <div 
                role="dialog"
                aria-modal="true"
                aria-labelledby="cartesian-variation-modal-title"
                className="relative bg-white dark:bg-slate-900 w-full max-w-2xl h-[80vh] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-300 border border-slate-100 dark:border-slate-800"
            >
                {/* Header */}
                <div className="px-8 py-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between shrink-0">
                    <div>
                        <h3 id="cartesian-variation-modal-title" className="text-xl font-black text-slate-800 dark:text-slate-100 tracking-tight flex items-center gap-3">
                            <i className="bi bi-stack text-blue-600" aria-hidden="true" />
                            Gerar variação
                        </h3>
                    ...
                    <div className="flex items-center gap-3">
                        <button 
                            ...
                            onClick={onClose} 
                            aria-label="Fechar"
                            className="p-2 text-slate-400 hover:text-red-500 disabled:opacity-30 transition-colors bg-slate-50 dark:bg-slate-800 rounded-xl cursor-pointer"
                        >
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                    <div className="mb-6 p-4 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/50 rounded-2xl flex items-center gap-4">
                        <i className="bi bi-info-circle-fill text-blue-600 text-lg" aria-hidden="true" />
                                                ...
                                                {(dragProvided, snapshot) => (
                                                    ...
                                                    >
                                                        <div className="flex gap-4 items-start">
                                                            ...
                                                            >
                                                                <i className="bi bi-grip-vertical text-xl" aria-hidden="true" />
                                                            </div>
                                                            <div className="flex-1 space-y-4">
                                                                <div className="flex items-center justify-between gap-4">
                                                                    <div className="flex-1 space-y-2">
                                                                        <label htmlFor={`cartesian-attr-select-${idx}`} className="text-[9px] font-black uppercase tracking-widest text-slate-400">Atributo</label>
                                                                        <select
                                                                            id={`cartesian-attr-select-${idx}`}
                                                                            value={opt.name}
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        className={`mt-6 px-4 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer ${opt.showName ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}
                                                                        title={opt.showName ? 'Mostrar nome no título' : 'Ocultar nome no título'}
                                                                    >
                                                                        <i className={`bi ${opt.showName ? 'bi-eye-fill' : 'bi-eye-slash-fill'} mr-1.5`} aria-hidden="true" />
                                                            ...
                                                            <button
                                                                ...
                                                                className="w-10 h-10 flex items-center justify-center text-slate-300 hover:text-red-500 transition-colors shrink-0 mt-6 cursor-pointer"
                                                            >
                    ...
                    <button
                        ...
                        className="w-full mt-6 py-4 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-3xl text-slate-400 hover:text-blue-600 hover:border-blue-100 dark:hover:border-blue-900/30 transition-all flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest cursor-pointer"
                    >
                ...
                <div className="p-8 border-t border-slate-50 dark:border-slate-800 flex gap-4 shrink-0 bg-white dark:bg-slate-900">
                    <button
                        ...
                        onClick={onClose}
                        disabled={isGenerating}
                        className="flex-1 py-4 bg-slate-50 dark:bg-slate-800 text-slate-400 rounded-2xl font-black uppercase tracking-widest text-[10px] disabled:opacity-50 cursor-pointer"
                    >
                        Cancelar
                    </button>
                    <button
                        ...
                        className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-blue-200 dark:shadow-none transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
                    >
                        {isGenerating ? (
                            <><i className="bi bi-hourglass-split animate-spin mr-2" aria-hidden="true" /> Gerando...</>
                        ) : (
                            <><i className="bi bi-magic mr-2" aria-hidden="true" /> Gerar Combinações</>
                        )}
                    </button>
                </div>
            </div>
