...
import { getOrdersByProductId } from '@/pages/utils/orderHistoryService';
import { formatCurrency } from '@/pages/utils/formatters';

interface ProductSalesTargetLike extends Partial<Product> {
    readonly variationId?: string;
    readonly parentId?: string;
}

interface AssistanceOrderItemLike {
    readonly id?: string;
...
export interface ProductSalesModalProps {
    readonly product: Product;
    readonly onClose: () => void;
}

/**
 * Modal para listagem e auditoria de pedidos de venda/assistência vinculados a um produto ou variação.
 */
export const ProductSalesModal: React.FC<ProductSalesModalProps> = ({ product, onClose }) => {
    const [orders, setOrders] = useState<Order[]>([]);
    const [loading, setLoading] = useState(true);

    const target = product as ProductSalesTargetLike;
    const variationId = target.variationId || (product.isVariation ? product.id : undefined);
    const productId = String(target.parentId || product.id || '');
    ...
    useEffect(() => {
        ...
        const fetchOrders = async () => {
            ...
            try {
                ...
                console.error('Falha ao carregar pedidos vinculados ao produto:', error);
            } finally {
                if (isMounted) {
                    setLoading(false);
                }
    ...
    const getQuantityInOrder = (order: Order) => {
        ...
        const assistanceItemsList = (order.assistanceItems || []) as readonly AssistanceOrderItemLike[];
        const assistanceItems = assistanceItemsList
            .filter((item) => {
                return variationId
                    ? String(item.variationId || '') === String(variationId)
    ...
    return createPortal(
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="product-sales-modal-title"
        >
            <div
            ...
            >
                ...
                <div className="px-10 py-8 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-slate-800/30">
                    <div>
                        <h2 id="product-sales-modal-title" className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight flex items-center gap-3">
                            <i className="bi bi-receipt text-blue-600" aria-hidden="true" />
                            Pedidos Vinculados
                        </h2>
                    ...
                    <button
                        onClick={onClose}
                        type="button"
                        className="p-3 hover:bg-white dark:hover:bg-slate-700 rounded-2xl transition-all text-slate-400 hover:text-red-500 border border-transparent hover:border-slate-200 dark:hover:border-slate-600 cursor-pointer"
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center bg-slate-50/50 dark:bg-slate-800/10 rounded-[3rem] border-2 border-dashed border-slate-100 dark:border-slate-800">
                            <div className="w-20 h-20 bg-white dark:bg-slate-800 rounded-3xl flex items-center justify-center mb-6 shadow-xl shadow-slate-200/50 dark:shadow-none">
                                <i className="bi bi-search text-4xl text-slate-200" aria-hidden="true" />
                            </div>
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                                    {orders.map((order) => {
                                        ...
                                        const allItems = [
                                            ...(order.items || []),
                                            ...((order.assistanceItems || []) as readonly AssistanceOrderItemLike[]),
                                        ];
                                        const item = allItems.find((i) => {
                                            return variationId
                                                ? String(i.variationId || '') === String(variationId)
                                                : String(i.productId || ('id' in i ? (i as { id?: string }).id : '') || '') === productId;
                                        });

                                        const typeColor =
                                            order.orderType === 'assistance'
                                                ? 'bg-purple-600 text-white shadow-purple-500/20'
                                                : order.orderType === 'showroom'
                                        ...
                                        return (
                                            <tr
                                                key={order.id}
                                                className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors group text-slate-700 dark:text-slate-300"
                                            >
                                                ...
                                                <td className="px-6 py-6">
                                                    <div className="flex flex-col">
                                                        <span className="text-xs font-bold line-clamp-1">
                                                            {order.customerData?.fullName || 'Cliente Geral'}
                                                        </span>
                                                        <span className="text-[10px] font-medium text-slate-400 mt-1">
                                                            {order.customerData?.cpfCnpj || 'S/ CPF/CNPJ'}
                                                        </span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-6 py-6">
                                                    <span
                                                        className={`text-[9px] font-black uppercase tracking-widest px-3 py-2 rounded-lg shadow-sm ${typeColor}`}
                                                    >
                                                        {order.orderType === 'assistance'
                                                            ? 'Assistência'
                                                            : order.orderType === 'showroom'
                                        ...
                                        <td className="px-6 py-6 text-center">
                                            <div className="flex flex-col items-center">
                                                <span className="text-sm font-black text-blue-600 dark:text-blue-400">
                                                    {orders.reduce((sum, order) => sum + getQuantityInOrder(order), 0)}{' '}
                                                    {product.unit || 'un'}
                                                </span>
