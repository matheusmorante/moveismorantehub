...
import { migrateProductReferences } from '@/pages/utils/productService';
import { toast } from 'react-toastify';

export interface ProductMigrationModalProps {
    readonly sourceProduct: Product;
    readonly onClose: () => void;
    readonly onSuccess: () => void;
}

/**
 * Modal para migrar histórico, movimentações e referências de um produto legado para um novo cadastro.
 */
export const ProductMigrationModal: React.FC<ProductMigrationModalProps> = ({
    sourceProduct,
    onClose,
    onSuccess,
}) => {
    ...
    const handleMigrate = async () => {
        ...
        const confirmed = window.confirm(
            `Tem certeza que deseja migrar todas as referências de "${sourceProduct.description}" para "${targetProduct.description}${
                targetVariation ? ` (${targetVariation.name})` : ''
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'Erro ao realizar migração.';
            toast.error(message);
        } finally {
    ...
    return (
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="product-migration-modal-title"
        >
            <button
                type="button"
            ...
            />
            <div className="relative bg-white dark:bg-slate-900 w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200">
                {/* Header */}
                <div className="px-8 py-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/30">
                    <div>
                        <h2 id="product-migration-modal-title" className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">
                            Migrar Referências
                        </h2>
                        <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">
                            Mover histórico entre produtos
                        </p>
                    </div>
                    <button
                        type="button"
                        onClick={onClose}
                        disabled={isMigrating}
                        className="p-2 hover:bg-white dark:hover:bg-slate-700 rounded-xl transition-all border border-transparent hover:border-slate-200 dark:hover:border-slate-600 text-slate-400 hover:text-slate-600 disabled:opacity-50 cursor-pointer"
                        aria-label="Fechar modal"
                    >
                ...
                <div className="p-8 space-y-6">
                    ...
                    <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/30 p-5 rounded-2xl flex gap-4">
                        <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/40 rounded-full flex items-center justify-center shrink-0">
                            <i className="bi bi-exclamation-triangle-fill text-amber-600 dark:text-amber-500 text-lg" aria-hidden="true" />
                        </div>
                        <div>
                            <p className="text-sm font-bold text-amber-800 dark:text-amber-400">
                                Atenção: Integridade de Dados
                            </p>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        <div className="flex justify-center -my-2 relative z-10">
                            <div className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center shadow-lg shadow-blue-500/30 border-4 border-white dark:border-slate-900">
                                <i className="bi bi-arrow-down-short text-2xl" aria-hidden="true" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">
                                Produto de Destino (Novo Vínculo)
                            </label>
                            <ProductAutocomplete
                                onSelect={(p, v) => {
                                    setTargetProduct(p);
                            ...
                            {targetProduct && (
                                <div className="mt-2 px-3 py-2 bg-blue-50/50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/30 rounded-xl flex items-center gap-2 transition-all animate-in slide-in-from-top-1">
                                    <i className="bi bi-check-circle-fill text-blue-500 text-sm" aria-hidden="true" />
                                    <span className="text-[11px] font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wide">
                                        Destino selecionado: {targetProduct.description}{' '}
                                        {targetVariation ? `(${targetVariation.name})` : ''}
                                    </span>
                                </div>
                            )}
                ...
                <div className="px-8 py-6 bg-slate-50/50 dark:bg-slate-800/30 border-t border-slate-50 dark:border-slate-800 flex justify-end gap-3">
                    <button
                        ...
                        onClick={onClose}
                        disabled={isMigrating}
                        className="px-6 py-2.5 text-xs font-black uppercase tracking-widest text-slate-500 hover:text-slate-700 transition-colors disabled:opacity-50"
                    >
                        Cancelar
                    </button>
                    <button
