        ...
        <div ref={containerRef} className="relative w-full mt-1">
            <div className="relative flex items-center">
                ...
                <input
                    ...
                    placeholder={placeholder || `Pesquisar ou selecionar ${fieldName}...`}
                    className={`w-full pl-6 pr-6 py-2 bg-transparent border-b-2 border-t-0 border-x-0 rounded-none text-xs outline-none transition-all ${
                        disabled
                            ? 'border-emerald-300 dark:border-emerald-700 text-emerald-600 dark:text-emerald-400 font-bold cursor-not-allowed'
                            : isInvalid
                            ? 'border-red-500 text-red-600 dark:text-red-400 placeholder:text-red-300'
                            : 'border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 placeholder:text-slate-400 focus:border-blue-600 dark:focus:border-blue-400'
                    } ${!disabled && hasSelectedValue ? 'font-bold' : ''}`}
                />
