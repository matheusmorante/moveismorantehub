...
- Name: products\categories-deletion-safety.spec.ts >> Suíte Permanente E2E: Ambientes e Categorias >> Cenário 1 — Criar Ambiente: modal, cadastro, exibição e persistência após reload
- Location: tests\e2e\products\categories-deletion-safety.spec.ts:360:5
...
Call log:
  - waiting for getByTestId('btn-new-environment')
...
```ts
  ...
  264 |         });
  265 | 
  266 |         await page.route('**/rest/v1/product_categories*', async route => {
  267 |             const req = route.request();
  268 |             const method = req.method().toUpperCase();
  269 |             const url = req.url();
  270 |             const matchCat = url.match(/category_id=eq\.([^&]+)/);
  271 | 
  272 |             if (matchCat) {
  273 |                 const cId = decodeURIComponent(matchCat[1]);
  274 |                 const filtered = currentProductCategories.filter(pc => pc.category_id === cId);
  275 |                 await route.fulfill({
  276 |                     status: 200,
  277 |                     headers: { 'content-range': `0-${filtered.length}/${filtered.length}` },
  278 |                     contentType: 'application/json',
  279 |                     body: JSON.stringify(filtered)
  280 |                 });
  281 |                 return;
  282 |             }
  283 | 
