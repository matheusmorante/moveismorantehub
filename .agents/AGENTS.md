...
- **Regra de Finalidade Automática para Despesas Operacionais (Salários, Veículos, Estoque, Impostos, Fretes) e Linguagem Natural do Assistente Financeiro**:
  - Despesas genuinamente operacionais da empresa (**Salários**, adiantamentos salariais, folha de pagamento, férias, 13º, comissões, combustível, manutenção de veículos operacionais, frete, compra de estoque, matéria-prima, fornecedores, impostos, DAS, FGTS, taxas fiscais e aluguel do galpão/loja) possuem **finalidade `BUSINESS` (Operação da Empresa) conhecida automaticamente**.
