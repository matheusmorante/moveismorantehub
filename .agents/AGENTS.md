...
- **Cabeçalho de Recibos e Papéis de Pedido Impressos (`ReceiptPage/Header` e `OrderPage/OrderHeader`)**:
  - No topo do **Recibo Impresso** e da **Folha/Papel de Pedido**, é exibida **apenas a logo oficial**, sem texto com o nome "Móveis Morante" ou slogan ao lado da logo, pois a imagem da logo já carrega integralmente a identidade visual da marca.
  - O container da logo (`header-logo-container`) não possui **borda ou sombra** (`border-0 shadow-none`), garantindo exibição limpa e uniforme no documento e na impressão.
- **Preservação de Ações Pós-Venda ao Editar Pedidos (`isButtonsClicked`)**:
  - Ao carregar um pedido existente para edição (`useSalesOrderForm` / `OrderEditModal`), o mapa de botões pós-venda já clicados (`isButtonsClicked`) é estritamente carregado e preservado no estado do formulário e na montagem do payload de atualização (`updatedOrder`).
  - Salvar edições em um pedido nunca reseta nem limpa o status de "check" verde das ações já realizadas (impressão de recibos, envio de WhatsApp, etiquetas, etc.).
- **Padronização do Selo de Oportunidade / Queima dos Salvados (ERP e Mobile)**:
  - O selo da oportunidade **Queima dos Salvados** (`condition === 'salvado'`, `is_salvado === true` ou `opportunity/opportunityName/opportunityId` de salvados) é padronizado em **todos os locais** do ERP e do App Mobile (lista de produtos, detalhes do pedido `ItemsFinancialInfo`, e tela de detalhes do pedido mobile `OrderDetailsSections`).
  - Ele deve sempre exibir o nome completo **"Queima dos Salvados"** (e nunca apenas "Salvado") acompanhado do ícone de fogo (`bi-fire` no ERP e `Flame` no Mobile), fundo âmbar (`bg-amber-100 dark:bg-amber-950`), borda âmbar (`border-amber-300 dark:border-amber-700`) e texto âmbar escuro (`text-amber-800 dark:text-amber-300`).
  - Apenas o selo dos posts de marketing possui a sua identidade gráfica própria de arte/feed. Em todos os demais módulos do ERP e do App Mobile, a exibição é 100% idêntica e unificada.
...
- **Pacote ZIP de Criativos para IA (`post-context.zip`) — Compatibilidade com ChatGPT e LLMs**:
  - Para evitar restrições, bloqueios de leitura de arquivos `.md` ou tratamentos chatos por parte do ChatGPT e de outras LLMs, os arquivos textuais de orientação no pacote ZIP são disponibilizados em formato de texto puro (`.txt`):
    - **`prompt.txt`**: Prompt completo, estruturado e pronto para uso da IA.
    - **`INSTRUCOES.txt`**: Diretrizes, explicações das pastas e resumo do produto em formato de texto puro universal.
    - O arquivo `INSTRUCOES.md` é mantido simultaneamente para garantir total retrocompatibilidade e navegação com formatação caso aberto em editores de markdown.
- **Sincronização de DF-e / NSU da SEFAZ (`sefaz-inbound-sync`)**:
  - A consulta do webservice `NFeDistribuicaoDFe` por NSU é executada no servidor através da Edge Function `sefaz-inbound-sync` no Supabase (não diretamente pelo navegador, devido a restrições de mTLS e CORS).
  - O controle do último NSU consultado e do status da sincronização é persistido na tabela `sefaz_nsu_control`.
  - O certificado A1 (`.pfx`) e a senha são lidos prioritariamente das Secrets do Supabase (`SEFAZ_CERTIFICATE_BASE64`, `SEFAZ_CERTIFICATE_PASSWORD`) com fallback automático para os dados gravados nas configurações do ERP (`settings` id `app`).
  - No frontend (`inboundInvoicesService.ts`), a sincronização invoca a Edge Function via `supabase.functions.invoke('sefaz-inbound-sync')` com fallback resiliente.
...
- **Regra de Finalidade Automática para Despesas Operacionais (Salários, Veículos, Estoque, Impostos, Fretes, Empréstimos e Financiamentos) e Linguagem Natural do Assistente Financeiro**:
  - Despesas genuinamente operacionais da empresa (**Salários**, adiantamentos salariais, folha de pagamento, férias, 13º, comissões, combustível, manutenção de veículos operacionais, frete, compra de estoque, matéria-prima, fornecedores, impostos, DAS, FGTS, taxas fiscais, aluguel do galpão/loja, **empréstimos e financiamentos da empresa**) possuem **finalidade `BUSINESS` (Operação da Empresa) conhecida automaticamente**.
...
- **Políticas RLS e Testes E2E de Transações Manuais Mobile (`financial_transactions`)**:
  - A tabela `financial_transactions` possui políticas permissivas de `SELECT`, `INSERT`, `UPDATE` e `DELETE` (`USING (true)` / `WITH CHECK (true)`), garantindo que operações manuais e de rotinas via Web/App funcionem sem rejeições silenciosas de RLS.
  - A suíte Playwright E2E (`erp/tests/mobile-transactions.spec.ts`) valida o ciclo completo de transações manuais no Expo Web: criação de Saída (com PIX e Outras), criação de Entrada (com categoria Outras), Edição de valor/descrição e Exclusão com confirmação e timer de segurança de 3s.
  - Os botões de tipo de movimentação (`TransactionTypeSelector`) possuem `accessibilityRole="button"`, `testID` dedicado e manipulador `onClick` direto para responder instantaneamente e sem bloqueios no ambiente Web.
- **Padronização Oficial de Categorias de Saída (Despesas) e Posição da Categoria 'Outras'**:
  - As categorias de saída oficiais são: **`Combustível`**, **`Manutenção do Carro`** (categorias veiculares distintas que ativam seleção de veículo no mobile), **`Manutenção do Imóvel`**, **`Aluguel`**, **`Energia`**, **`Internet e Telefone`**, **`Seguros`**, **`Impostos e Taxas`**, **`Compra de Mercadorias (Boletos, PIXs de compra de móveis e frete de fornecedores)`**, **`Empréstimos e Financiamentos`** (apenas com finalidade empresarial / gastos da empresa `BUSINESS`), **`Materiais de Limpeza`**, **`Marketing e Publicidade`**, **`Materiais de Escritório`**, **`Pró-labore`**, **`Salários e Encargos`**, **`Taxas Bancárias`** e **`Outras`**.
  - A categoria **`Embalagens e Fretes`** foi removida.
  - **Invariante de Ordenação**: Qualquer categoria genérica com nome iniciando por "Outra" (ex: **`Outras`**) deve se posicionar **obrigatoriamente no final da listagem** tanto no ERP quanto no App Mobile (`sortCategoriesWithOthersAtEnd`).
...
- **Regra Conceitual de Variações**: No Morante Hub, **TODO produto tem pelo menos uma variação**. Um produto cadastrado sem atributos específicos (produto simples) é conceitualmente e operacionalmente a sua própria variação única principal (1 produto = 1 variação). Produtos com atributos (cor, tecido, tamanho) possuem múltiplas variações filhas cadastradas. Não existe conceito de produto sem variação no sistema.
