# Roteador de Skills Especializadas — Morante Hub

Consulte a skill correspondente em `.agents/skills/<skill-name>/SKILL.md` para o procedimento operacional detalhado:

| Categoria | Skill | Gatilho / Quando Utilizar |
|---|---|---|
...
| **Modelagem & Docs** | `modelagem-negocio-arquitetura` | Alterar processos, atualizar diagramas Mermaid e manter documentação viva em `docs/`. |
...
| **Organização de Pastas** | `organizacao-arquivos-diretorios` | Estruturação de diretórios, subpastas semânticas e migração segura de arquivos. |
| **Limpeza de Projeto** | `limpeza-projeto-segura` | Identificar e remover arquivos/pastas desnecessários, caches e lixos com segurança. |
| **Economia de Tokens** | `caveman` | Respostas ultra-comprimidas sem perda técnica para economizar cotas de LLM. |
| **TDAH / Produtividade** | `i-have-adhd` | Respostas focadas com próxima ação imediata, listas numeradas e zero prolixidade. |
