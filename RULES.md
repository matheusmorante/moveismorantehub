...
11. **Autoridade e Preenchimento da URL do Google Maps (`mapsUrl`) nos Endereços**:
    - **Preenchimento Exclusivamente Manual**: O campo de URL do Google Maps (`Link do Google Maps da Localização` / `mapsUrl`) nos formulários de cadastro e edição de pessoas/clientes (`PersonAddressSection`), pedidos (`ShippingData`) e assistência (`AssistanceCustomerSection`) é de preenchimento estritamente manual. Ele serve para quando o endereço não for localizado por rua/número/CEP.
    - **Proibição de Preenchimento Automático pelo Autocomplete**: Ao clicar em uma sugestão de endereço no logradouro (`AddressAutocompleteInput`), o campo de URL do Google Maps **NUNCA** deve ser preenchido ou sobrescrito automaticamente.
