  ...
  useEffect(() => {
    const check = async () => {
      ...
      try {
        ...
        const url = config?.androidUpdateUrl || 'https://expo.dev/artifacts/eas/GoCr7Fvu_0gV7-45jS-MLYYX7Yax7xwcWxpGuxQfNyM.apk';
        const installedBuild = getInstalledAndroidBuild();

        // Se a build instalada for diferente da build requerida oficial (13), bloqueia para atualização obrigatória
