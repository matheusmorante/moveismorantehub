...
export function MobileOrderCard({ order, dark, handlingOptions, onDetails }: Props) {
  ...
  return <TouchableOpacity onPress={onDetails} style={[styles.card, dark && styles.cardDark]}>
    <View style={[styles.header, { backgroundColor: cancelled ? '#e2e8f0' : pickup ? '#f3e8ff' : '#d1fae5' }]}>
      {pickup ? <Package size={16} color={cancelled ? '#64748b' : '#a855f7'} /> : <Truck size={16} color={cancelled ? '#64748b' : '#10b981'} />}
      <View style={styles.badges}>{!cancelled && outside && <Text style={[styles.badge, styles.red]}>MONTAGEM FORA</Text>}{!cancelled && internal && <Text style={[styles.badge, styles.orange]}>MONTAGEM DEPÓSITO</Text>}<Text style={[styles.status, cancelled && styles.cancelledStatus]}>● {statusLabel(order.status || data.status || 'Agendado')}</Text></View>
    </View>
