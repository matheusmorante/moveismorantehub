...
import { ITEMS_PER_PAGE } from './stockMovesService';
...
export const fetchInboundInvoicesPage = async ({ page, searchTerm, dateFilter }: {
    ...
    let query = supabase
        .from('inbound_invoices')
        .select('id,numero_nfe,serie,chave_acesso,emitente_nome,emitente_cnpj,data_emissao,valor_total,status_recebimento,itens,xml_conteudo,inbound_invoice_items(*)', { count: 'exact' });
    ...
    const invoices = (data || []).map((row: any) => {
        ...
        const isSummaryOnly = items.length === 0 && Boolean(row.xml_conteudo?.includes('<resNFe') || !row.xml_conteudo);
...
export const fetchInboundInvoiceForMappings = async (invoiceId: string) => {
    const { data, error } = await supabase
        .from('inbound_invoices')
        .select('id, numero_nfe, serie, emitente_nome, emitente_cnpj, supplier_id, itens, xml_conteudo, inbound_invoice_items(item_number, product_code, product_description, product_id, variation_id, raw_item)')
        .eq('id', invoiceId)
        .maybeSingle();
    ...
    let items = (invoiceItems.length ? invoiceItems : normalizedItems.map((row: any) => ({
        ...(row.raw_item || {}),
    ...
    // O ERP repara itens incompletos usando o XML original antes de abrir o gerenciamento.
    if (data.xml_conteudo && items.some((item: any) => !item.productDescription || item.productDescription === 'Item sem descrição' || (Number(item.unitCost || 0) === 0 && Number(item.totalCost || 0) === 0))) {
        try {
            const parsed = parseInboundNfeXml(data.xml_conteudo);
            if (parsed.items?.length) {
                items = parsed.items.map((xmlItem: any, index: number) => {
                    const existing = items.find((current: any) => current.itemNumber === xmlItem.itemNumber || (current.productCode && current.productCode === xmlItem.productCode)) || items[index];
                    return {
                        ...xmlItem,
                        ...
                        sellingPrice: existing?.sellingPrice ?? xmlItem.sellingPrice,
                        linkMode: existing?.linkMode || xmlItem.linkMode,
                        compositionLinks: existing?.compositionLinks || existing?.composition_links || xmlItem.compositionLinks || xmlItem.composition_links,
                    };
                });
            }
        } catch (repairError) {
            console.warn('[StockInvoiceService] Não foi possível reparar itens pelo XML:', repairError);
        }
    }
...
export const updateInboundInvoiceMappings = async (
    ...
    // A listagem usa a tabela normalizada quando ela existe; mantenha-a em
    // sincronia para que os vínculos salvos não continuem aparecendo como
    // pendentes após reabrir a tela.
    if (items.length) {
        const itemRows = items.map((item: any, index: number) => ({
            invoice_id: invoiceId,
            item_number: Number(item.itemNumber ?? item.item_number ?? index + 1),
            product_id: item.matchedProductId || null,
            variation_id: item.matchedVariationId || null,
            raw_item: item,
        }));
